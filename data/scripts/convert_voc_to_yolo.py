...
def main():
    ap = argparse.ArgumentParser(description="Convert VOC (PVELAD + PV-MD) to YOLO format for YOLOv8.")
    ap.add_argument("--pvelad-img", default="PVELAD/EL2021/trainval/JPEGImages")
    ap.add_argument("--pvelad-ann", default="PVELAD/EL2021/trainval/Annotations")
    ...
    ap.add_argument("--out", default="data/detection_yolo")
    args = ap.parse_args()

    voc_roots = [
        (Path(args.pvelad_img), Path(args.pvelad_ann)),
    ...
    print(f"YOLO dataset ready at: {out_root} | classes: {len(classes)}")
